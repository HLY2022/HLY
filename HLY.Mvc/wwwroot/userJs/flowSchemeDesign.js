new Vue({
    ...
    mounted() {
        ...
        layui.config({
            base: "/js/"
        }).use(['form', 'ztree', 'layer', 'utils', 'element', 'jquery', 'cookie','table', 'droptree', 'HLY', 'flow/gooflow', 'flowlayout'], function () {
            var form = layui.form,
            ...
            var changeForm = function (id) { //切换表单
                ...
                $.getJSON("/forms/get?id=" + id, function (data) {
                    if (data.Result.FrmType == 0) {
                        ...
                        _this.frmPreview = '复杂表单暂时只能在<a href="http://demo.HLY.net.cn:1803">企业版</a>查看，开源版预计会在以后的开源版本中发布'
                    }
                });
            }
