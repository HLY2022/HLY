new Vue({
    ...
    mounted() {
        ...
        layui.config({
            base: "/js/"
        }).use(['form', 'dtree', 'layer', 'utils', 'element', 'jquery', 'cookie','table', 'slimscroll', 'HLY', 'flow/gooflow', 'flowlayout'], function () {
            var form = layui.form,
                    ...
                    $.getJSON("/forms/get?id=" + record.FrmId, function (data) {
                        if (data.Code != 500) {
                            if (data.Result.FrmType == 0) {
                                ...
                                $("#frmPreview").html('复杂表单暂时只能在<a href="http://demo.HLY.net.cn:1803">企业版</a>查看，开源版预计会在以后的开源版本中发布');
                            }
                        }
                    });
