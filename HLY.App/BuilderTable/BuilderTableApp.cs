...
namespace HLY.App
{
    public class BuilderTableApp : BaseStringApp<BuilderTable,HLYDBContext>
    {
        ...
        public BuilderTableApp(IUnitWork<HLYDBContext> unitWork, IRepository<BuilderTable,HLYDBContext> repository,
        ...
        private void GenerateApp(BuilderTable sysTableInfo, List<BuilderTableColumn> sysColumns)
        {
            ...
            {
                throw new Exception("未找到HLY.app类库,请确认是否存在");
            }
        ...
        private void CreateEntityModel(List<BuilderTableColumn> sysColumns, BuilderTable tableInfo)
        {
            ...
            FileHelper.WriteFile(
                mapPath +
                $"\\HLY.Repository\\Domain\\", tableInfo.ClassName + ".cs",
                domainContent);

            string HLYDBContextPath = mapPath + "\\HLY.Repository\\HLYDBContext.cs";
            FileHelper.RegxAddContentByParenthesis(HLYDBContextPath, "public virtual DbSet<" + tableInfo.ClassName + "> " + tableInfo.TableName + "s { get; set; }");
        }
        ...
        public void CreateVue(CreateVueReq req)
        {
            ...
            {
                throw new Exception("请提供vue项目的根目录,如：C:\\HLY.Pro\\Client");
            }
            var sysTableInfo = Repository.FirstOrDefault(u => u.Id == req.Id);
