...
namespace HLY.App.Test
{
    public class TestBase
    {
        ...
        public void Init()
        {
            ...
            //读取HLY.WebApi的配置文件用于单元测试
            var path = AppContext.BaseDirectory;
            int pos = path.IndexOf("HLY.App");
            if (pos == -1) //如果测试入口是HLY.WebApi
            {
                pos = path.IndexOf("HLY.WebApi");
            }
            var basepath = Path.Combine(path.Substring(0,pos) ,"HLY.WebApi");
            ...
            httpContextAccessorMock.Setup(x => x.HttpContext.Request.Query[Define.TENANT_ID]).Returns("HLYDBContext");
            serviceCollection.AddScoped(x => httpContextAccessorMock.Object);
            
            serviceCollection.AddDbContext<HLYDBContext>();
            ...
            Console.WriteLine($"单元测试数据库信息:{dbtypes[httpContextAccessorMock.Object.GetTenantId()]}/{config.GetSection("ConnectionStrings")["HLYDBContext"]}");
