...
//                policy.AllowAnyHeader().AllowAnyMethod().AllowCredentials().WithOrigins(origins)));
            ...
            var dbType = dbtypes["HLYDBContext"];
            if (dbType == Define.DBTYPE_SQLSERVER)
            {
                services.AddDbContext<HLYDBContext>(options =>
                    options.UseSqlServer(Configuration.GetConnectionString("HLYDBContext")));
            }
            else if(dbType == Define.DBTYPE_MYSQL) //mysql
            {
                services.AddDbContext<HLYDBContext>(options =>
                    options.UseMySql(Configuration.GetConnectionString("HLYDBContext"),new MySqlServerVersion(new Version(8, 0, 11))));
            }
            else  //oracle
            {
                services.AddDbContext<HLYDBContext>(options =>
                    options.UseOracle(Configuration.GetConnectionString("HLYDBContext"), o=>o.UseOracleSQLCompatibility("11")));
            }
